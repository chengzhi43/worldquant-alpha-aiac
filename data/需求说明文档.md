...
- 缺少“限流与重试策略”：平台接口可能出现 429（Too Many Requests），需要在数据拉取与模拟阶段做退避与重试，并把失败归因结构化。
...
- 多样性选择（Diversity Gate）：
  ...
  - 需要最少覆盖以下多样性维度（可配置权重）：
    - Region/Universe 多样性：同日尽量覆盖多个 region 或 universe（若配置了多 region）。
...
- 禁用清单：
  ...
  - 禁用算子（例如你不希望使用的算子类别）
- 期望的多样性配额：例如“每周覆盖至少 6 个 dataset、至少 3 个 region”。
...
2. v1（多样性驱动）：实现 dataset/field 自动探索与分层采样；实现多样性选择器（预算 3–4/天）。
...
create table if not exists forum_user (
  ...
  platform text not null default 'wqb_forum_zh',
  user_external_id text,
  display_name text,
  profile_url text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (platform, user_external_id)
);
...
create table if not exists forum_post (
  ...
  url text not null,
  title text,
  author_user_id bigint references forum_user(id),
...
create table if not exists forum_comment (
  ...
  parent_comment_id bigint references forum_comment(id) on delete cascade,
  author_user_id bigint references forum_user(id),
  created_time timestamptz,
  raw_html text,
  text_content text,
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (platform, comment_external_id)
);
...
create table if not exists forum_attachment (
  ...
  comment_id bigint references forum_comment(id) on delete cascade,
  media_type text not null, -- image / file
  source_url text,
  minio_bucket text,
...
create table if not exists extracted_alpha_expression (
  id bigserial primary key,
  platform text not null default 'wqb_forum_zh',
  post_id bigint references forum_post(id) on delete cascade,
  comment_id bigint references forum_comment(id) on delete cascade,
  expression text not null,
  expression_language text default 'FASTEXPR',
  context_text text,
  extracted_from text not null, -- post / comment / codeblock
...
create table if not exists rag_chunk (
  ...
  post_id bigint references forum_post(id) on delete cascade,
  comment_id bigint references forum_comment(id) on delete cascade,
  attachment_id bigint references forum_attachment(id) on delete cascade,
...
- Metadata：至少包含 `chunk_id`（对应 `rag_chunk.id`）、`source_type`、`post_id/comment_id/attachment_id`、`url`、`title`、`created_time`、`language`
